...
public class CpuInfo {
	...
	private static final String LOG_TAG = "MobileTester-" + CpuInfo.class.getSimpleName();
		...
		try {
			...
			if (isInitialStatics) {
				...
				if (isPositive(processCpuRatio) && isPositive(totalCpuRatio.get(0))) {
					...
					TesterService.bw.write(mDateTime2 + Constants.COMMA + ProcessInfo.getTopActivity(context) + Constants.COMMA +heapData+ pMemory
